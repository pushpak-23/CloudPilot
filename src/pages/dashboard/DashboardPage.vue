<template>
  <div class="p-8 space-y-8 max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-zinc-400 mt-1">
          Live status control plane for OpenStack resources and system
          hypervisors.
        </p>
      </div>
      <div
        class="hidden md:flex items-center gap-2 text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3.5 py-1.5 rounded-full"
      >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <RouterLink
        to="/compute"
        class="group bg-zinc-900/60 border border-zinc-800/80 hover:border-blue-500/30 rounded-xl p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-blue-500/3 flex flex-col justify-between h-36"
      >
        <div class="flex justify-between items-start">
          <span
            class="text-sm font-semibold text-zinc-400 group-hover:text-zinc-300"
            >Compute Instances</span
          >
          <Server
            class="text-zinc-500 group-hover:text-blue-500 transition-colors"
            :size="20"
          />
        </div>
        <div class="mt-4">
          ...
          >
            {{ computeStore.totalInstancesCount }}
          </div>
          <div class="text-[11px] text-zinc-500 mt-1">
            {{ computeStore.runningCount }} Active,
            {{ computeStore.stoppedCount }} Shutoff
          </div>
        </div>
      </RouterLink>
      ...
      >
        ...
        <div class="mt-4">
          ...
          <div class="text-[11px] text-zinc-500 mt-1">
            {{ (storageStore.totalAllocatedGb / 1024).toFixed(1) }} TB total
            allocated
          </div>
        </div>
      </RouterLink>
      ...
      >
        ...
        <div class="mt-4">
          <div
            class="text-4xl font-black text-white group-hover:text-blue-400 transition-colors"
          >
            {{ identityStore.totalProjectsCount }}
          </div>
          <div class="text-[11px] text-zinc-500 mt-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="space-y-6 lg:col-span-1 flex flex-col justify-between">
        <div
          class="bg-zinc-900/40 border border-zinc-800 rounded-xl p-6 space-y-6"
        >
          <div class="flex items-center justify-between">
          ...
          <div class="space-y-5">
            ...
            <div class="space-y-1.5">
              <div class="flex justify-between text-xs font-semibold">
                ...
                <span class="text-white font-mono"
                  >{{ monitoringStore.metrics.cpuLoad }}%</span
                >
              </div>
              <div
            ...
            <div class="space-y-1.5">
              <div class="flex justify-between text-xs font-semibold">
                ...
                <span class="text-white font-mono">
                  {{ monitoringStore.metrics.memoryAllocatedGb }} GB /
                  {{ monitoringStore.metrics.memoryTotalGb }} GB
                </span>
              </div>
              <div
                class="w-full bg-zinc-950 rounded-full h-2 overflow-hidden border border-zinc-800"
            ...
            <div class="space-y-1.5">
              <div class="flex justify-between text-xs font-semibold">
                ...
                <span class="text-emerald-400 font-mono"
                  >{{ monitoringStore.metrics.networkBps }} Gbps</span
                >
              </div>
              <div
                class="w-full bg-zinc-950 rounded-full h-2 overflow-hidden border border-zinc-800"
              >
                <div
                  ...
                  :style="{
                    width: `${(monitoringStore.metrics.networkBps / 20) * 100}%`,
                  }"
                ></div>
              </div>
            </div>
        ...
        <div
          class="bg-zinc-900/40 border border-zinc-800 rounded-xl p-6 flex flex-col justify-between flex-1"
        >
          <div>
            <h2 class="font-bold text-lg text-white mb-4">Cluster Health</h2>
            <div
              class="flex items-center gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-400 mb-4"
            >
              <CheckCircle :size="24" class="shrink-0" />
              <div>
                <div class="text-sm font-bold">Services Nominal</div>
                <div class="text-xs text-emerald-500/80">
                  All openstack-nodes online.
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div>
                <div class="text-sm font-bold">
                  {{ monitoringStore.metrics.activeAlerts }} Active Alerts
                </div>
                <div class="text-xs text-red-500/80">
                  Critical alert pending in Ceph pools.
                </div>
              </div>
            </div>
          </div>
