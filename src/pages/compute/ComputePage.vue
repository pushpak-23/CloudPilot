<template>
  <div class="p-8 space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-extrabold tracking-tight text-white bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">
          Compute Control Plane
        </h1>
        <p class="text-zinc-400 mt-1">Manage virtual machines, flavors, Glance images, hypervisors, and bulk automated deployments.</p>
      </div>
    ...
    <!-- Modals -->
    <LaunchWizardModal :show="showLaunchModal" @close="showLaunchModal = false" />
    <CreateFlavorModal :show="showCreateFlavorModal" @close="showCreateFlavorModal = false" />
    <CreateKeypairModal :show="showCreateKeyModal" @close="showCreateKeyModal = false" />
    <ServerDetailsModal :show="serverDetailsModal" :id="selectedServerId" @close="serverDetailsModal = false" />
  </div>
</template>
...
const activeTab = ref<'instances' | 'flavors' | 'images' | 'hypervisors' | 'keypairs' | 'wizard'>('instances')
